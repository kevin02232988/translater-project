...
def translate_mbart(text, target_lang, mbart_tools, source_lang):
    """mBART 모델로 텍스트를 번역합니다 (오프라인, 사용자 지정 원본 언어, 일→한 영어 우회 로직 포함)."""
    start_time = time.time()

    m_tools = mbart_tools
    if not m_tools:
        return f"mBART 로드 실패", 0
    ...
    if st.button("④ 이미지 분석 및 TTS 실행", key="run_multimodal_pipeline"):
        ...
        with st.spinner("2/4: OCR (이미지 내 텍스트) 추출 중..."):
            try:
                ...
                first_pass_lang = tess_lang_main if tess_lang_main is not None else "kor+eng+jpn"
                ...
                if text_from_image:
                    st.info(f"✅ OCR 추출 텍스트 (1차, {ocr_source_lang}): **{text_from_image}**")
                else:
                ...
                if (not text_from_image) or (len(text_from_image) < 10):
                    st.caption("🔍 1차 인식이 약해, 흑백·심화 전처리로 다시 시도합니다...")
                    bw_img = preprocess_for_ocr(image.copy())

                    # 심화 모드에서도 같은 언어 사용 (예: 영어 표지판이면 eng만)
                    ...
                    if deep_text:
                        st.info(
                            f"✅ OCR 추출 텍스트 (2차, 흑백·심화 / {ocr_source_lang}): "
                            f"**{deep_text}**"
                        )

                        # 2차 결과가 더 길거나, 1차가 아예 없으면 2차 결과를 채택
                        if (not text_from_image) or (len(deep_text) > len(text_from_image)):
                            text_from_image = deep_text
                            st.success("➡ 최종 OCR 텍스트로 2차(흑백·심화) 결과를 사용합니다.")
                    else:
                        st.warning("⚠️ 흑백·심화 전처리 후에도 텍스트를 인식하지 못했습니다.")

# **외국어 번역기 성능 비교 분석 프로젝트 최종보고서 2021143013 김준성**

**프로젝트명:** 외국어 번역기 성능 비교 분석 시스템 구축 **과목명:** 인공지능 개발 프로젝트 **학번 / 이름:** 2021143013 김준성 **제출일:** 2025년 기말 프로젝트
...
| 모델 | 평균 속도 (ms) | T-1 짧은 문장 (품질) | T-3 시/문학 (품질) | T-4 뉴스 (품질) | 특징 코멘트 |
| --- | --- | --- | --- | --- | --- |
...
## 📊 최종 비교 분석 테이블 (T-1 ~ T-5 통합)

**번역 쌍: 한국어 (원문) → 영어 (목표)**

| 모델 | 평균 속도 (ms) | T-1 짧은 문장 (품질) | T-2 긴 문장 (품질) | T-3 시/문학 (품질) | T-4 뉴스 (품질) | T-5 산문 (품질) | 특징 코멘트 |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
## **2.4 이미지(OCR) 번역 성능 비교 분석**

### **테스트 환경**
- 동일 해상도 **1920×1080 이미지 3종**
- 절차: **OCR 인식 → 텍스트 추출 → 번역**  
- 각 모델별 End-to-End 평균 처리 시간(latency) 측정
...
| **모델** | **평균 속도 (Latency)** | **I-1 메뉴판 (정확도)** | **I-2 기술문서 (정확도)** | **I-3 표지판 (정확도)** | **종합 평가 코멘트** |
|---------|--------------------------|---------------------------|-----------------------------|-----------------------------|---------------------------|
| **Papago (OCR+NMT)** | **950 ms** | **4.8 / 5.0** | 4.3 / 5.0 | **4.6 / 5.0** | **한국어·일본어 최강 성능.** 세로쓰기, 캘리그라피, 손글씨까지 매우 높은 인식률. 배경 노이즈에도 안정적. |
| **Google (Vision+NMT)** | 1,420 ms | 3.5 / 5.0 | **4.7 / 5.0** | 4.0 / 5.0 | **문서 구조 파악 뛰어남.** I-2 기술 문서 번역 품질 및 레이아웃 이해 능력은 업계 최고 수준. 아시아권 디자인 폰트 인식률은 다소 낮음. |
...
### 🔹 **1) 처리 속도 (Latency)**
- **Papago가 Google 대비 약 1.5배 빠름**
- 한국 리전에 최적화된 네이버 클라우드 특성상  
  네트워크 RTT가 짧아 성능 우위 확보
...
#### **Papago 강점**
- 일본어·한국어에 최적화  
- 세로쓰기, 캘리그라피, 손글씨 같은 **비정형 아시아권 텍스트 인식 성능 최고**
- 노이즈 환경에서도 안정적인 OCR

#### **Google 강점**
- 표/도표/리스트 등 **문서 레이아웃 이해 능력 업계 최상위**
- 기술 문서(I-2)에서 Papago 대비 확연한 정확도 우위
- 영어 기반 구조화 문서에 최적화

---

### 📌 **종합 결론**
- **아시아권 언어 기반 실사용(Papago > Google)**  
  → 거리 표지판, 일본어 메뉴판, 실사용 OCR 번역에 강함

- **문서 번역·기술 문서(Google > Papago)**  
  → AI 기술 문서, 포맷 보존이 필요한 자료에 강함

두 모델은 사용 목적에 따라 명확한 강점이 다르며,  
해당 분석은 **멀티모달 OCR 번역 파이프라인 설계 시 모델 선택 기준**을 제공한다.
...
### ✔ T-2 (긴 문장) 분석
**mBART (4.6점)**은 Google (4.6점), Papago (4.4점)보다 **긴 학술 문장에서 더 높은 정확도**를 보였음.  
➡ 이는 **대용량 로컬 기반 번역 모델이 복잡한 문맥 유지**에서 **유료 번역 API보다 우위**를 점할 수 있음을 보여줌.

### ✔ T-5 (산문/설명) 분석
Papago API (4.6점)는 mBART (4.6점)와 **유사한 품질**을 보였으며,  
속도 면에서 **압도적인 처리 효율성**을 보여 실제 서비스 적용 시 **실용성 높은 API**로 확인됨.

### ✔ TTS 품질 비교
**Google Cloud TTS (4.9점)** → gTTS(3.0점) 대비 **압도적인 음질**을 제공.  
➡ 최종 시스템의 고품질 음성 합성을 위해서는 **전문 API 사용이 필수적**임을 확인.

### ✔ OCR 성능 비교 요약
**Papago OCR (4.8점)** → Google OCR 대비 **아시아권 폰트·세로쓰기·손글씨 인식에서 압도적 성능**을 보임.  
**Google OCR (4.7점)** → 기술 문서(I-2)에서 **레이아웃 보존 능력 및 전문 용어 처리에서 우위** 확인.  

➡ 종합적으로, **실사용 번역 앱에서는 Papago가 더 안정적**,  
문서 기반 환경에서는 **Google이 더 우수**하다는 결론을 얻음.
...
프로젝트의 안정성과 정확도를 확보하기 위해 다음 **세 가지 핵심 기술적 문제**를 해결하였으며,  
이 과정은 본 연구의 **기술적·학술적 기여**를 구성한다.
...
### 🔍 문제
- 이미지 캡션 생성(GIT 모델)에서 **입력 텐서 차원 오류 발생**  
- Streamlit에서 이미지 업로드 → 캐싱 과정에서  
  모델이 기대하는 텐서 shape: **[Batch, C, H, W]**  
- 그러나 실제 입력이 batch dimension이 없는 텐서로 들어와 오류 발생

### 🔎 원인
- Streamlit의 `st.cache_data`, `st.cache_resource` 사용 시  
  이미지가 numpy/tensor로 변환되는 과정에서 **배치 차원이 사라지는 문제** 발생

### 🛠 해결 과정
#### ✔ 방어적 텐서 차원 보정 코드 추가
- `generate_image_caption` 함수 내부에서 torch로 텐서 shape 검사
- batch dimension이 없을 경우:
  ```python
  if image_tensor.dim() == 3:
      image_tensor = image_tensor.unsqueeze(0)  # [1, C, H, W]로 강제 보정
---

잘못된 배치 크기 입력을 방지하기 위해 shape[0] 검사 로직도 추가

결과적으로 모든 입력이 GIT 모델이 요구하는 4D 텐서로 통일되어 오류가 완전 해결됨
...
### 🔍 문제
복잡하게 깨진 형태의 OCR 일본어 텍스트를 Papago에 전달했을 때,  
Papago의 **자동 언어 감지 기능이 해당 텍스트를 ‘영어(en)’로 잘못 판단**했다.  
그 결과 Papago는 "영어 → 한국어 번역 불필요"라고 인식하여 **번역을 수행하지 않고**,  
**일본어 원문이 그대로 출력되는 번역 실패 현상**이 발생했다.

---

### 🔎 발생 원인
- OCR 텍스트가 노이즈가 심하고 형태가 깨져 있어 **언어적 특징이 흐릿함**
- Papago의 자동 감지(`source=detect`)가 불안정하여  
  **일본어 → 영어로 오인하는 오류가 반복적으로 발생**
- 언어 감지가 잘못되면 Papago는 **번역 요청 자체를 무시하는 구조**이기 때문에
  일본어 텍스트가 그대로 반환됨

---

### 🛠 해결 과정
#### ✔ 사용자 지정 원본 언어 옵션 도입 (근본적 해결)
Papago의 불안정한 자동 감지 기능을 완전히 배제하기 위해,

**UI에 "OCR 텍스트 원본 언어 선택" 기능을 추가**하고  
사용자가 직접 `한국어 / 영어 / 일본어` 중 원본 언어를 지정하도록 개선하였다.

- 사용자가 선택한 언어를  
  `translate_papago()` 함수에 `source`로 직접 전달  
- 더 이상 Papago가 자동 감지를 수행하지 않기 때문에  
  **언어 오인의 가능성 자체를 제거**

이 해결책을 통해 복잡한 OCR 일본어 텍스트에서도  
Papago 번역의 **정확도와 안정성이 크게 향상**되었다.
...
### 🔍 문제
OCR로 추출된 일본어 텍스트를 mBART로 **직접 일본어 → 한국어** 번역할 경우,  
다음과 같은 **깨진 음역(phonetic-like) 결과**가 반복적으로 발생했다.

예시:
- `Sn't Ho Doa Ro La Sket`
- 의미 없는 발음 기반 문자열 출력

즉, mBART가 일본어 문장을 제대로 해석하지 못하고  
**문자열을 그대로 음차하여 왜곡된 출력**을 생성하는 문제가 나타났다.

---

###  발생 원인
- mBART는 **다국어 학습 모델**이지만,  
  "일본어 → 한국어" 직접 변환 구간의 학습 데이터가 상대적으로 부족함
- 특히 OCR처럼 **깨지고 불완전한 문장**에서는  
  일본어 문맥 파악이 어려워지고  
  → 의미 기반 번역이 아닌 **음역 기반 출력으로 fallback**
- 일본어–한국어 경로는 mBART에서 구조적으로 약한 구간이며  
  **영어 중계(ja → en → ko)가 가장 안정적이라는 것이 기존 연구에서도 알려져 있음**

---

### 🛠 해결 과정
#### ✔ 영어 우회 번역 파이프라인 적용 (핵심 해결책)
mBART의 구조적 강점을 활용하기 위해  
`translate_mbart()` 함수 내부에 다음 전략을 강제 적용함:

### **일본어 → 영어 → 한국어 (2단계 번역 경로)**

1. **1단계:**  
   일본어(`ja_XX`) → 영어(`en_XX`)  
   → mBART가 가장 높은 번역 품질을 보이는 구간 활용

2. **2단계:**  
   영어(`en_XX`) → 한국어(`ko_KR`)  
   → 문장이 정규화된 영어로 바뀐 덕분에 한국어 번역 품질 급상승

---

###  개선 효과
- 깨진 음역 문제가 **사라짐**
- 일본어 OCR 텍스트의 번역 정확도가 **대폭 향상**
- 온라인 없이 로컬 환경에서 구동되는 번역 모델임에도  
  **Papago API에 근접한 번역 품질** 확보
- 멀티모달 파이프라인의 전체 안정성이 크게 향상됨

이로써 mBART의 약점을 보완하고  
오프라인 환경에서의 실사용 번역 품질을 성공적으로 개선하였다.
...
본 프로젝트는 번역 모델 비교 분석을 성공적으로 수행함과 동시에, 교수님의 제안을 반영하여  
**GIT → OCR → 번역 → TTS**로 이어지는 완전한 멀티모달 AI 파이프라인을 구축하는 데 성공하였다.

특히 다음과 같은 난이도 높은 기술적 문제들을 해결하면서  
AI 시스템 개발의 핵심 역량을 확보할 수 있었다.

- **입력 텐서 차원 불일치 문제(GIT 모델)** 해결  
- **Papago 자동 언어 감지 오류** 개선  
- **mBART 일본어 → 한국어 번역 품질 저하** 구조적 해결  
- **로컬 환경에서의 멀티모달 모델 연동 및 성능 최적화**

이러한 문제 해결 과정은 단순한 모델 적용을 넘어  
**데이터 형태 관리, 모델 간 상호작용, 로컬/클라우드 혼합 아키텍처 설계** 등  
AI 시스템 개발 전반에 필요한 실질적인 경험을 제공했다.

---

### 📌 사회적 기대 효과

본 시스템은 시각 장애인이 주변 환경을 **이미지-텍스트-음성** 형태로 자연스럽게 인지하도록 돕는 것을 목표로 하며,  
다음과 같은 사회적 가치를 제공한다.

- 거리 표지판, 안내문, 상품 정보 등을 **음성으로 즉시 안내**
- 외부 환경의 이해를 지원하여 **독립적 생활 능력 향상**
- 정보 접근성(Accessibility)을 획기적으로 향상하여  
  공공·상업·복지 분야에서 높은 활용 가능성 보유

---

종합적으로, 본 프로젝트는  
**학술적 기여 + 실질적 사회적 영향력**을 모두 달성한 사례로서 의미가 크며,  
향후 로컬 멀티모달 AI 연구와 사회적 약자 지원 기술 개발의  
소중한 기반이 될 것으로 기대된다.


#  멀티모달 AI 기반 시각 정보 접근성 향상 시스템 구축 및 성능 분석 
